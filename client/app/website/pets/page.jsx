              ...
              <div className="bg-white rounded-lg p-1 shadow-sm border flex gap-1">
                {["grid", "list"].map((mode) => (
                  <button key={mode} type="button" onClick={() => setViewMode(mode)}
                    className={`px-3 py-2 lg:px-4 lg:py-3 rounded-md text-md font-medium transition-all duration-200 flex items-center justify-center ${viewMode === mode ? "bg-blue-500 text-white shadow-md" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100 dark:hover:bg-dark-card"}`}>
                    {mode === "grid"
          ...
          <div className={viewMode === "grid" ? "grid gap-1 xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 -mt-[5px] lg:mt-4" : "flex flex-col space-y-4 -mt-[5px] lg:mt-4"}>
            {isLoading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="mt-2 text-md text-gray-500">{t("dashboard.pets.adjustFilters", "Try adjusting your filters to find more pets.")}</p>
              </div>
            )}
          </div>
